...
import { adapterDB, clientsDB } from "./db";
...
class NeAdapter {
  ...
  async upsert(id, payload, expiresIn) {
    //console.log("INSERT", this.name, id)
    if (await adapterDB.asyncFindOne({ _id: this.id(id) })) {
      return await adapterDB.asyncUpdate({ _id: this.id(id), }, { ...payload, expiresIn: Date.now() + expiresIn })
    }
    return await adapterDB.asyncInsert({ ...payload, _id: this.id(id), id, expiresIn: Date.now() + expiresIn })
    /**
     *
  ...
  async find(id) {
    ...
    if (this.name === 'Client') {
      const found = await clientsDB.asyncFindOne({ client_id: id }) as any
      //console.log('FIND', this.name, id, found ? found.client_id : null)
      return {...found, redirectUri: found.callbacks, redirect_uris: found.callbacks}
    }

    const found = await adapterDB.asyncFindOne({ _id: this.id(id) }) as any
    //console.log('FIND', this.name, id, !!found)

    if (!found) return null
    // if (found.expiresIn < Date.now()) {
    //   //console.log('EXPIRED', this.name, id, found.expiresIn)
    //   await adapterDB.asyncRemove({ _id: this.id(id) })
    //   return null
    // }
    return found
  }
  ...
  async findByUserCode(userCode) {
    const found = adapterDB.asyncFindOne({ uid: userCode }) as any
    if (!found) return null
    if ((new Date(found.expiresIn)).getTime() < Date.now()) {
      await adapterDB.asyncRemove({ uid: userCode })
      return null
    }
    return found
  }
   ...
   */
  async findByUid(uid) {
    const found = adapterDB.asyncFindOne({ uid: uid }) as any
    if (!found) return null
    if ((new Date(found.expiresIn)).getTime() < Date.now()) {
      await adapterDB.asyncRemove({ uid: uid })
      return null
    }
    return found
  }
   ...
   */
  async consume(id) {
    await adapterDB.asyncUpdate({ _id: this.id(id) }, { consumed: true })
  }
  ...
  async revokeByGrantId(grantId) {
    await adapterDB.asyncRemove({ grantId: grantId })
  }
}
